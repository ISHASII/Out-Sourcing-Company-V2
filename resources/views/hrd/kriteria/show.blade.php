...
@section('dashboard-title', 'Kriteria: ' . $categoryModel->name)
...
<div class="space-y-6 animate-fade-in" x-data="{ 
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <a href="{{ route('hrd.kriteria.index') }}" class="p-2 rounded-xl bg-slate-50 text-slate-500 hover:text-[#003d7c] hover:bg-blue-50 transition border border-slate-200">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div>
                <h3 class="text-xl font-bold text-slate-800 tracking-tight">Kriteria {{ $categoryModel->name }}</h3>
                <p class="text-xs text-slate-500 mt-1">Kelola kualifikasi khusus untuk kategori pekerjaan ini.</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h4 class="text-sm font-extrabold text-slate-800">Daftar Pengaturan Kriteria</h4>
                <p class="text-[11px] text-slate-400 mt-0.5">Total: {{ $criteria->total() }} Kriteria</p>
            </div>
            
            {{-- Search Bar --}}
            <form action="{{ route('hrd.kriteria.show', $categoryModel->name) }}" method="GET" class="w-full sm:w-72 relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kriteria..." 
                ...
                @if(request('search'))
                    <a href="{{ route('hrd.kriteria.show', $categoryModel->name) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-rose-500">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('hrd.kriteria.destroy', $c) }}" method="POST" class="inline"
                                          x-data @submit.prevent="$dispatch('open-confirm-modal', {
                                              ...
                                              message: 'Kriteria {{ addslashes($c->label) }} akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.',
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-slate-400 italic">
                                Belum ada kriteria yang terdaftar untuk kategori ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-show="showAddModal" 
         ...
         <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden transform transition"
             ...
             <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                 <h4 class="text-sm font-extrabold text-slate-800">Tambahkan Kriteria</h4>
             ...
             <form action="{{ route('hrd.kriteria.store') }}" method="POST" class="p-6 space-y-4">
                 @csrf
                 <input type="hidden" name="category" value="{{ $categoryModel->name }}">
                 ...
                 <div class="pt-2 flex gap-3">
                     <button type="button" @click="showAddModal = false"
                             class="flex-1 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition">
                         Batal
                     </button>
                     <button type="submit"
                             class="flex-1 px-4 py-2.5 bg-[#003d7c] hover:bg-[#002f60] text-white font-bold text-xs rounded-xl transition shadow-md hover:shadow-lg">
                         Simpan Kriteria
                     </button>
                 </div>
             </form>
    ...
    <div x-show="showEditModal" 
         ...
         <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden transform transition"
             ...
             <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                 <h4 class="text-sm font-extrabold text-slate-800">Ubah Pengaturan Kriteria</h4>
                 <button @click="showEditModal = false" class="text-slate-400 hover:text-slate-600">
                     <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                 </button>
             </div>
             <form :action="`/kriteria/${editId}`" method="POST" class="p-6 space-y-4">
                 @csrf
                 @method('PUT')
                 ...
                 <div>
                     ...
                     <input type="text" name="label" x-model="editLabel" required
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-[#003d7c] focus:ring-1 focus:ring-[#003d7c] text-sm font-semibold">
                 </div>

                 <div>
                     <label class="text-xs font-bold text-slate-600 block mb-1.5">Status Default <span class="text-rose-500">*</span></label>
                     <select name="default_status" x-model="editStatus" required
                             class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-[#003d7c] focus:ring-1 focus:ring-[#003d7c] text-sm bg-white font-semibold">
                 ...
                 <div>
                     ...
                     <input type="number" name="default_weight" x-model="editWeight" min="0" max="100" required
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-[#003d7c] focus:ring-1 focus:ring-[#003d7c] text-sm font-semibold text-slate-800">
                 </div>
